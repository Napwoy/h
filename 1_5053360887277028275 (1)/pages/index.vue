                ...
                <div class="header__inner">
                    ...
                    <v-dialog v-model="dialog" max-width="300">
                        <template #activator="{ on, attrs }">
                            <div v-if="!controllerInfo || controllerInfo.PSD" class="header__btn" disabled>
                        ...
                        <div class="connect-modal__content">
                            <div class="connect-modal__content-inner">
                                ...
                                <div class="connect-modal__links">
                                    <div :disabled="providerType === 'M'" @click="clickConnectMetamask" class="connect-modal__link btn">
                                        METAMASK
                                    </div>

                                    <div :disabled="providerType === 'W'" @click="clickConnectWalletconnect" class="connect-modal__link btn">
                                        CONNECTWALLET
                                    </div>
                                </div>
                            </div>
            ...
            <main class="main">
                <div class="swapper">
                            ...
                            <div class="swapper__col">
                                ...
                                >
                                    ...
                                    <template v-slot:selection="{ item }">
                                      <img v-if="FromToken === 'DBX Smart Network (DBX-20)'" src="/dbx.svg" />
                                      <img v-if="FromToken === 'Ethereum (ERC-20)'" src="/Ethereum.svg" />
                                      <img v-if="FromToken === 'Binance Smart Chain (BEP-20)'" src="/Binance.svg" />
                                      
                                      <span class='select-btn__output-txt'>{{ item }}</span>
                                    </template>

                                   <template v-slot:item="{ item }">
                                      <img v-if="item === 'DBX Smart Network (DBX-20)'" src="/dbx.svg" />
                                      <img v-if="item === 'Ethereum (ERC-20)'" src="/Ethereum.svg" />
                                      <img v-if="item === 'Binance Smart Chain (BEP-20)'" src="/Binance.svg" />
                                      {{ item }}
                                  </template>
                                </v-select>

                                <div class="swapper-quantity">
                                    <div class="swapper-quantity__item">Balance: <span>{{ BNStrToNumstr(dirBalance[0]) }} DBX</span></div>
                                    <div class="swapper-quantity__item">Approved: <span>{{ BNStrToNumstr(currentApproved) }} DBX</span></div>
                                    <div class="swapper-quantity__item">Fee: <span>{{ controllerInfo ? BNStrToNumstr(feeBNStr) : '...' }} DBX</span></div>
                                    <div class="swapper-quantity__item">Min: <span>{{ BNStrToNumstr(minBNStr) }} DBX</span></div>
                                </div>
                            </div>
                            <div class="swapper__col">
                                <v-select 
                                  style="height: 70px;" 
                                  outlined :value="ToToken" 
                                  :items="Tokens" 
                                  :disabled="!!loading_swapping" 
                                ...
                                >
                                    <v-icon slot="append" size="12">fa fa-chevron-down</v-icon>
                                    <template v-slot:selection="{ item }">
                                      <img v-if="ToToken === 'DBX Smart Network (DBX-20)'" src="/dbx.svg" />
                                      <img v-if="ToToken === 'Ethereum (ERC-20)'" src="/Ethereum.svg" />
                                      <img v-if="ToToken === 'Binance Smart Chain (BEP-20)'" src="/Binance.svg" />
                                       <span class='select-btn__output-txt'>{{ item }}</span>
                                    </template>
                                ...
                                <div class="swapper-quantity">
                                    <div class="swapper-quantity__item">Balance: <span>{{ BNStrToNumstr(dirBalance[1]) }} DBX</span></div>
                                    <div class="swapper-quantity__item">
                                        Will receive:
                                        <span>
                                            {{ amountEnough && controllerInfo ? '~' + BNStrToNumstr(substractFee(amountBN.toString(), feeBNStr).toString()) : '...' }} DBX
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="swapper__amount">
                            <div class="swapper-amount">
                                ...
                                <div class="swapper-amount__field-wrapper">
                                  ...
                                  <v-text-field v-else outlined label="Choose the vacancy" disabled />
                                </div>
                            </div>
                        </div>

                        <button class="swapper__controls" :loading="loading_swapping" v-if="requestDisabled" disabled @click="handleClickRequest">
                            <span class="btn swapper__link">
                                SWAP
                            </span>
                        </button>
                        <button class="swapper__controls" :loading="loading_swapping" v-else @click="handleClickRequest">
                            <span class="btn swapper__link">
                                SWAP
                            </span>
                        </button>
                    ...
                    <v-card-text v-if="hashes && hashes_url && hashes_names">
                        ...
                        <a :href="`${hashes_url.C}/tx/${hashes.txHashCollect}`" target="_blank" rel="noopener noreferrer">
                            {{ hashes.txHashCollect }}
                        </a>
                        <br />
                        Dispense ({{ hashes_names.D }})
                        <a :href="`${hashes_url.D}/tx/${hashes.txHashDispense}`" target="_blank" rel="noopener noreferrer">
                ...
                <div 
                v-if="loadingMessage || errorMessage || warningMessage || inBetween || hashes"
                ...
                >
                  ...
                  <v-alert
                    ...
                    type="info"
                  >
                  Value sent. Sending request to controller. It might take up to 5min, please wait...
                  </v-alert>
...
const _providerB = new providers.JsonRpcProvider("https://bsc-dataseed.binance.org/"); // for reading contracts
const _providerE = new providers.InfuraProvider(1, "976b8b2358be48468b36d8739e79414e"); // for reading contracts
  ...
  params: [
    {
      ...
      iconUrls: ['https://cryptologos.cc/logos/binance-coin-bnb-logo.svg', 'https://cryptologos.cc/logos/binance-coin-bnb-logo.png'],
    },
  ],
}

const Tokens = ['Ethereum (ERC-20)', 'Binance Smart Chain (BEP-20)', 'DBX Smart Network (DBX-20)']
...
export default Vue.extend({
  ...
  computed: {
    ...
    connectButtonLabel(): string {
      return this.wallet ? this.wallet.substr(0, 6) + '...' + this.wallet.substr(-4) : 'Connect'
    },
    ...
    err_chainId(): boolean {
      return !!this.chainId && this.chainId !== { EB: 1, BE: 56, EX: 1, BX: 56, XE: 5348, XB: 5348 }[this.direction]
    },
    ...
    errorMessage(): string {
      return this.err_controllerInfo
        ...
        : this.err_chainId
        ? `You selected wrong network for this direction. Select ${this.dirChainName[0]} in your provider${
    ...
    // CONVERTERS
    amountBN(): BigNumber {
      return this.amountValid ? numstrToBN(this.inputAmount.trim()) : BigNumber.from(0)
    },
  ...
  methods: {
      ...
      try {
        ...
        const wc = new WalletConnectProvider(
          this.direction === 'EB' || this.direction === 'EX'
            ? { rpc: { 1: 'https://mainnet.infura.io/v3/f91771c75ee2409198e8ea8561bfaea8' }, chainId: 1, qrcode: true }
            : this.direction === 'XB' || this.direction === 'XE'
            ? { rpc: { 5348: 'https://dbxnode.com' }, chainId: 5348, qrcode: true }
            : { rpc: { 56: 'https://bsc-dataseed.binance.org/' }, chainId: 56, qrcode: true }
        )
        await wc.enable()
        ...
        this.errorMessage_misc = 'Could not connect Walletconnect. Error: ' + error.message
        console.error(error)
      }
        ...
        switch (chainId) {
          case '0x1':
            if (this.ToToken == 'Ethereum (ERC-20)') {
              ...
              this.direction = (this.direction[1] + this.direction[0]) as DirectionType
            }
    ...
    async connectMetamask() {
      ...
      try {
        ...
        this.errorMessage_misc = 'Could not connect MetaMask. Error: ' + error.messagee
        console.error(error)
      }
      this.loading_provider = false
    },
    restoreInputAmount() {
      this.inputAmount = removeTrailingZeros(BNStrToNumstr(this.currentApproved, 18))
    },
    handleClickAppend() {
      if (this.approvedEnough && !this.approvedEqual) this.restoreInputAmount()
      else if (this.canMaximizeAmount) this.inputAmount = removeTrailingZeros(BNStrToNumstr(this.dirBalance[0], 18))
    },
    ...
    async loadControllerInfo() {
      ...
      try {
        this.controllerInfo = (await Axios.get('https://dbxbridge.uc.r.appspot.com/info')).data
        if (!this.controllerInfo) throw new Error('Received invalid data from controller')
      } catch (error) {
        console.error(error)
      }
    ...
    async handleClickRequest() {
      ...
      try {
        ...
        if (
          ...
          ((this.direction === 'XE' || this.direction === 'XB') && this.approvedX_dirE !== (this.direction === 'XE'))
        ) {
          await this.approve()
          this.inBetween = true
        }
    ...
    async approve() {
      ...
      if (this.direction === 'XE' || this.direction === 'XB') {
        const ptx = await BAX.populateTransaction.writeEntry(this.direction === 'XE')
    ...
    async requestSwap() {
      ...
      try {
        this.hashes = (await Axios.get(`https://dbxbridge.uc.r.appspot.com/process?direction=${this.direction}&address=${this.wallet}`)).data
